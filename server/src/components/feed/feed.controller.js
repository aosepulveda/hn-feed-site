...
module.exports = {
  ...
  reset: async (req, res) => {
    try {
      ...
      const feeds = await Feed.collection.insert(filteredHits);
      return res.status(HttpStatus.OK).json(feeds.ops.map((feed) => {
        return {
          id: feed.objectID,
          title: feed.title ? feed.title : feed.story_title,
          createdAt: feed.created_at,
      ...
      logger.error('something bad happen when i tried to reset feeds.', { stack: e.stack });
      return res
